...
def function(seconds: int):  # объявляем функцию с олним параметром
    ...
    def minutes() -> int:  # объявляем функцию без параметра которая буде возвращать количество минут
        return (seconds - days() * (24 * 60 * 60) - hours() * (60 * 60)) // 60  # функция, которая возвращает число

    def seconds__() -> int:  # объявляем функцию без параметра которая буде возвращать количество секунд
        return (seconds - days() * (24 * 60 * 60) - hours() * (60 * 60) - minutes() * 60) % 60

    print(f'{days()} дней, {hours()} часов, {minutes()} минут,{seconds__()} секунд')
...
def math_(x_: int) -> int:  # объявляем функцию с одним параметром, которая возвращает число
    if -5 <= x_ <= 5:  # условие
        y: int = x_ ** 2  # переменная, которая ссылается на значение вычисленное в уравнении
        return y  # функция, которая возвращает число
    elif x_ < -5:  # альтернативное условие
        y: int = 2 * abs(x_) - 1  # переменная, которая ссылается на число
        return y  # функция, которая возвращает число
    elif x_ > 5:  # альтернативное условие
        y: int = 2 * x_
        return y
    ...
    def list_() -> tuple:
        ...
        for elem in whatever:  # цикл: для каждого элемента в списке
            if type(elem) == int:  # условие: если тип данных объекта - целое число
                ...
                count_letters_ += len(elem)  # добавляем длину слова к значению переменной
        return count_int, count_letters_  # функция, которая возвращает два кортеж из двух значений
...
for object_ in list_of_everything:  # цикл: для каждого объекта в списке
    home_work(object_)  # вызываем функцию и передаем ей аргумент, выводим в кансоль работу функции
